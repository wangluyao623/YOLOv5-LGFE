#直接拿的D-Net中的DLK，未改动 0.605左右，上涨一个点
...
class CB2d(nn.Module):
    ...
    def forward(self, x):
        ...
        context = x
        if self.channel_mul_conv is not None:
            middle = self.channel_mul_conv(context)
...
class DLK(nn.Module):
    ...
    def forward(self, x):   
        att1 = self.att_conv1(x) #1,32,64,64
        att2 = self.att_conv2(att1) #1,32,64,64

        att = torch.cat([att1, att2], dim=1) #1,64,64,64
        ...
        # t1 = att[:,0,:,:] #1,64,64
        # t11 = t1.unsqueeze(1) #1,1,64,64
        # p1 = att[:,1,:,:] #1,64,64
        # t22 = p1.unsqueeze(1) #1,1,64,64
        # t2 = att1 * t1  #1,32,64,64
        # p2 = att2*p1 #1,32,64,64
