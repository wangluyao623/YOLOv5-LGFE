...
# import sys  #如果只是运行yolo.py的时候就要加上这两句话
# sys.path.append('/data1/lsl/wly/YOLO/yolov5-master_2')
...
class TAU(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv0 = nn.Conv2d(in_channels, in_channels, 5, padding=2, groups=in_channels)  #depth-wise conv
        self.conv_spatial = nn.Conv2d(in_channels, in_channels, 7, stride=1, padding=9, groups=in_channels, dilation=3)   #conv_spatial
    ...
    def forward(self, x):# [1*32*32*32]
        # sa = self.dw_conv(x)                  #sa [1*32*32*32]
        # sa = self.pointwise(sa)               #sa [1*32*32*32]
        # sa = self.dw_d_conv(sa)               #sa [1*32*32*32]
        # sa = self.pointwise_conv(sa)          #sa [1*32*32*32]

        sa = self.conv0(x)                       #sa [1*32*32*32]
        sa = self.conv_spatial(sa)               #sa [1*32*32*32]
        sa = self.conv1(sa)                      #sa [1*32*32*32]



        b, c, _, _ = x.size()
        da = self.avg_pool(x).view(b, c)      #da [1, 32]
        da = self.fc(da).view(b, c, 1, 1)     #da [1, 32,1,1]
       
        da = da.sigmoid()                     ##da [1, 32,1,1]
...
class CoDEM2(nn.Module):
    ...
    def __init__(self,channel_dim):
        ...
        self.cv2 = nn.Conv2d(in_channels=self.channel_dim, out_channels=self.channel_dim, kernel_size=5, stride=1, padding=2, groups=self.channel_dim)
           
        
        self.cv4 = nn.Conv2d(2*channel_dim, 2*channel_dim, kernel_size=3, stride=1, padding=1)
        self.cv5 = nn.Conv2d(2*channel_dim,2*channel_dim, kernel_size=3, stride=1,padding=1)
        self.cv6 = nn.Conv2d(channel_dim, 2*channel_dim,3, 1, 1)
    ...
    def forward(self,x): #1,32,64,64
        x1 = self.cv1(x) #1 32 32 32
        x1_1 = self.tau(x1)#1 32 32 32
        x1_1 = F.interpolate(x1_1, scale_factor=2, mode='bilinear', align_corners=False)#1，32，64，64
        x2 = self.cv2(x)#1，32，64，64
