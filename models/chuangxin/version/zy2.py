...
#         self.cv1 = nn.Conv2d(in_channels, 2*in_channels, kernel_size=3, padding=1, stride=1)
#         self.cv2 = nn.Conv2d(2*in_channels, in_channels, kernel_size=3, padding=1, stride=1)
...
class DLK(nn.Module):
    ...
    #----------------------------------------------------------------------------
        x_h_1 = self.pool_h(x1)
        x_w_1 = self.pool_w(x1)
        x_gather_1 = x_h_1 + x_w_1
        ge_1 = self.excite(x_gather_1)
        out1 = x1 * ge_1
        out1 = self.cv2(out1 + x1)


        x_h_2 = self.pool_h(x2)
        x_w_2 = self.pool_w(x2)
        x_gather_2 = x_h_2 + x_w_2
        ge_2 = self.excite(x_gather_2)
        out2 = x2 * ge_2
        out2 = self.cv2(out2 + x2)
